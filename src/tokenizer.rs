...
pub type TokenList = LinkedList<(usize, Token)>;
...
impl StringProcessor for Tokenizer {
  fn process(mut self, line: &str) -> TokenList {
    for (position, character) in line.chars().enumerate() {
      ...
      if token != Token::Skip {
        self.tokens.push_back((position, token));
  ...
  fn implicit_multiplication(&mut self, position: usize, character: char) {
    if character == '(' || character.is_alphabetic() {
      if self.previous_state == State::NumberSign ||
        ...
        self.value.clear();
        self.tokens.push_back((self.value_position, Token::Operator('*')));
...
mod tests {
  ...
  #[test]
  fn test_brackets() {
    assert_eq!(tokenize("(").front(), Some(&(0, Token::BracketOpening)));
    assert_eq!(tokenize(")").front(), Some(&(0, Token::BracketClosing)));
  }
  ...
  fn test_operators() {
    ...
    assert_eq!(tokenize("2-2").iter().skip(1).next(), Some(&(1, Token::Operator('-'))));
    assert_eq!(tokenize("*").front(), Some(&(0, Token::Operator('*'))));
    assert_eq!(tokenize("/").front(), Some(&(0, Token::Operator('/'))));
    assert_eq!(tokenize("=").front(), Some(&(0, Token::Operator('='))));
    assert_eq!(tokenize("^").front(), Some(&(0, Token::Operator('^'))));
  }
  ...
  #[test]
  fn test_identifiers() {
    assert_eq!(tokenize("x").front(), Some(&(0, Token::Identifier("x".to_string()))));
    assert_eq!(tokenize("x123x").front(), Some(&(0, Token::Identifier("x123x".to_string()))));
    assert_eq!(tokenize("_foo_bar").front(), Some(&(0, Token::Identifier("_foo_bar".to_string()))));
    assert_eq!(tokenize("_foo_123").front(), Some(&(0, Token::Identifier("_foo_123".to_string()))));
  }

  #[test]
  fn test_numbers() {
    assert_eq!(tokenize("1").front(), Some(&(0, Token::Number(1.0))));
    assert_eq!(tokenize("1.23").front(), Some(&(0, Token::Number(1.23))));
    assert_eq!(tokenize(".23").front(), Some(&(0, Token::Number(0.23))));
  }

  #[test]
  fn test_numbers_scientific() {
    assert_eq!(tokenize("1e23").front(), Some(&(0, Token::Number(1.0e23))));
    assert_eq!(tokenize("1e-23").front(), Some(&(0, Token::Number(1.0e-23))));
    assert_eq!(tokenize("1.01e+23").front(), Some(&(0, Token::Number(1.01e23))));
  }

  #[test]
  fn test_expressions() {
    let mut t = tokenize("2+2");
    assert_eq!(t.pop_front(), Some((0, Token::Number(2.0))));
    assert_eq!(t.pop_front(), Some((1, Token::Operator('+'))));
    assert_eq!(t.pop_front(), Some((2, Token::Number(2.0))));

    let mut t = tokenize("2+x");
    assert_eq!(t.pop_front(), Some((0, Token::Number(2.0))));
    assert_eq!(t.pop_front(), Some((1, Token::Operator('+'))));
    assert_eq!(t.pop_front(), Some((2, Token::Identifier("x".to_string()))));

    let mut t = tokenize("x=2");
    assert_eq!(t.pop_front(), Some((0, Token::Identifier("x".to_string()))));
    assert_eq!(t.pop_front(), Some((1, Token::Operator('='))));
    assert_eq!(t.pop_front(), Some((2, Token::Number(2.0))));
  }
  ...
  fn test_implicit_multiplication() {
    ...
    assert_eq!(t.pop_front(), Some((0, Token::Number(2.0))));
    assert_eq!(t.pop_front(), Some((1, Token::Operator('-'))));
    assert_eq!(t.pop_front(), Some((2, Token::Number(-1.0))));
    assert_eq!(t.pop_front(), Some((2, Token::Operator('*'))));
    assert_eq!(t.pop_front(), Some((3, Token::Identifier("x".to_string()))));

    let mut t = tokenize("-2x");
    assert_eq!(t.pop_front(), Some((0, Token::Number(-2.0))));
    assert_eq!(t.pop_front(), Some((0, Token::Operator('*'))));
    assert_eq!(t.pop_front(), Some((2, Token::Identifier("x".to_string()))));

    let mut t = tokenize("-2(4)");
    assert_eq!(t.pop_front(), Some((0, Token::Number(-2.0))));
    assert_eq!(t.pop_front(), Some((0, Token::Operator('*'))));
    assert_eq!(t.pop_front(), Some((2, Token::BracketOpening)));
    assert_eq!(t.pop_front(), Some((3, Token::Number(4.0))));
    assert_eq!(t.pop_front(), Some((4, Token::BracketClosing)));
